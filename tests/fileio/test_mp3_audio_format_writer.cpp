...
TEST_F(AMP3AudioFormatReader,
       DISABLED_IsOpenReturnFalseIfOutputStreamOpenFailed) {
    auto bad_stream = std::make_unique<FileOutputStream>("/bad/path");
    ASSERT_FALSE(bad_stream->isOpen());

    MP3FormatWriter writer(std::move(bad_stream), sample_rate, num_channels,
                           num_bits);
...
TEST_F(AMP3AudioFormatReader, OpenFailedIfSampleRateUnsupported) {
    ...
    MP3FormatWriter writer(std::move(out_stream), unsupported_sample_rate,
                           num_channels, num_bits);
...
TEST_F(AMP3AudioFormatReader, OpenFailedIsNumChannelUnsupported) {
    ...
    MP3FormatWriter writer(std::move(out_stream), sample_rate, invalid_channels,
                           num_bits);
