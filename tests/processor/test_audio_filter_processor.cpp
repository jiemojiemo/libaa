...
{
public:
    AudioBlock generateExpectedOutput(const AudioBlock& in_block, const IIRFilterParameter& iir_params)
    {
        AudioBlock expected_output{in_block};
        AudioFilterUtilities utilities;
        utilities.updateCoeffes(iir_params, sample_rate);

        for(auto c = 0u; c < expected_output.audio_buffer.getNumberChannels(); ++c)
        {
            auto& b = utilities.biquad_array[c];
            float* in_out = expected_output.audio_buffer.getWriterPointer(c);
            for(auto i = 0u; i < expected_output.audio_buffer.getNumberFrames(); ++i)
            {
...
TEST_F(AAudioFilterProcessor, FilterTypeParameterAsExpected)
{
    ...
    ASSERT_THAT(param.getMaxPlainValue(), Eq(float(FilterType::kNumFilterType)));
}
...
TEST_F(AAudioFilterProcessor, CanProcessSamples)
{
    ...
    AudioBlock expected_output = generateExpectedOutput(in_block,
                                                        iir_params);
...
TEST_F(AAudioFilterProcessor, ParameterBeChangedIfThereComesParameterChangesEvent)
{
    AudioBlock in_block{{{1.0, 1.1, 1.2},
                         {2.0, 2.1, 2.2}}};
...
TEST_F(AAudioFilterProcessor, ProcessAsExpectedAfterParametChanges)
{
    ...
    float type_norm_value = p.getParameters().get(0).convertPlainValueToNormalizedValue(float(iir_params.type));
    float fc_norm_value = p.getParameters().get(1).convertPlainValueToNormalizedValue(iir_params.fc);
    float Q_norm_value = p.getParameters().get(2).convertPlainValueToNormalizedValue(iir_params.Q);
    float gain_norm_value = p.getParameters().get(3).convertPlainValueToNormalizedValue(iir_params.boost_or_cut_db);
