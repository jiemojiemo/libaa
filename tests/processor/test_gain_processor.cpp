...
public:
    void SetUp() override {
        in_block = AudioBlock({{1.f, 1.0f}, {2.f, 2.0f}});

        out_block = AudioBlock(in_block.audio_buffer.getNumberChannels(),
                               in_block.audio_buffer.getNumberFrames());
    }
    GainProcessor gain;
    AudioBlock in_block;
...
TEST_F(AGainProcessor, CanConstructWithDefault) { GainProcessor gain; }
...
TEST_F(AGainProcessor, ApplyDynamicGainIfThereHasParameterChange) {
    ...
    auto in_buffer = AudioBufferNew<float>({{1.f, 1.0f}, {2.f, 2.0f}});
    ...
    for (auto i = 0u; i < expected_out.getNumberFrames(); ++i) {
        float dynamic_normalized_gain =
            getRampValue(i, expected_out.getNumberFrames(),
                         normalized_start_gain, normalized_end_gain);
