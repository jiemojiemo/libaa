...
TEST_F(AParameterChanges, CanConstuctWithSizeOfParamterChange)
{
    ...
    ASSERT_THAT(param_changes.getParameterChangeSet().size(), Eq(num_param_change_buffer));
}
...
TEST_F(AParameterChanges, RingbufferHasIncreasedIndex)
{
    ...
    const ParameterChanges::ParameterChangeSet& ringbuffers = param_changes.getParameterChangeSet();
