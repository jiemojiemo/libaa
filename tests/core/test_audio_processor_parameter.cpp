...
public:
    void SetUp() override {
        param = make_unique<AudioProcessorParameter>(
            ParameterType::kFloat, param_id, test_name, default_value,
...
TEST_F(AAudioProcessorParameter, CanInitWithIDAndValues) {
    AudioProcessorParameter p(ParameterType::kFloat, param_id, test_name,
                              default_value, min_plain_value, max_plain_value);
}
...
TEST_F(AAudioProcessorParameter, DefaultValueSetToMinValueIfNotInRange) {
    param = std::make_unique<AudioProcessorParameter>(
        ParameterType::kFloat, param_id, "param_name", 100, -10, 10);

    ASSERT_THAT(param->getDefaultPlainValue(), Eq(param->getMinPlainValue()));
}

TEST_F(AAudioProcessorParameter,
       SetMinValueToMaxValueIfInitMinValueLargeThanMax) {
    param = std::make_unique<AudioProcessorParameter>(
        ParameterType::kFloat, param_id, "param_name", default_value, 100, 10);
...
TEST_F(AAudioProcessorParameter, CanGetNormalizedValueFromPlainValue) {
    ...
    float normalized_value = (plain_value - param->getMinPlainValue()) /
                             (max_plain_value - min_plain_value);

    ASSERT_THAT(param->convertPlainValueToNormalizedValue(plain_value),
                Eq(normalized_value));
}
...
TEST_F(AAudioProcessorParameter, CanGetPlainValueFromNormalizedValue) {
    ...
    float plain_value =
        (max_plain_value - min_plain_value) * normalized_v + min_plain_value;

    ASSERT_THAT(param->convertNormalizedValueToPlainValue(normalized_v),
                Eq(plain_value));
}
...
TEST_F(AAudioProcessorParameter, CanGetCurrentNormalizedValue) {
    ASSERT_THAT(param->getNormalizedValue(),
                Eq(param->convertPlainValueToNormalizedValue(
                    param->getDefaultPlainValue())));
}
...
TEST_F(AAudioProcessorParameter, CanConstructWithChoiceStrings) {
    int default_index = 0;
    param = std::make_unique<AudioProcessorParameter>(
        ParameterType::kChoice, param_id, "choices", default_index, 0,
        choices_strings.size(), choices_strings);
}

TEST_F(AAudioProcessorParameter,
       ChoiceStringCanConvertToNormalizedValueWithStep) {
    int default_index = 1;
    param = std::make_unique<AudioProcessorParameter>(
        ParameterType::kChoice, param_id, "choices", default_index, 0,
        choices_strings.size(), choices_strings);
...
TEST_F(AAudioProcessorParameter, CanConvertNormalizedValueToChoiceString) {
    ...
    ASSERT_THAT(param->convertNormalizedValueToChoiceString(norm_val),
                Eq(choices_strings[default_index]));
}

TEST_F(AAudioProcessorParameter,
       ConvertNormalizedValueReturnsEmptyIfChoiceStringIsEmpty) {
    int default_index = 1;
    choices_strings = {};
    param = std::make_unique<AudioProcessorParameter>(
        ParameterType::kChoice, param_id, "choices", default_index, 0,
