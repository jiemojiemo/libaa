...
public:
    ...
    vector<complex<float>> freq_in{
        {28, 0}, {-4, 9.65685425}, {-4, 4}, {-4, 1.65685425}, {-4, 0}};
...
TEST_F(AFFT, fftconvolve) {
    ...
    vector<float> conv_result =
        FFT::fftconvolve(x.data(), x.size(), y.data(), y.size());
    ...
    ASSERT_THAT(conv_result,
                Pointwise(NearWithPrecision(1e-6), expect_conv_result));
}