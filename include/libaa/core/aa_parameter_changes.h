...
public:
    using ParameterChangeSet =
        std::map<int, std::shared_ptr<ParameterChangeRingbuffer>>;
    ...
    explicit ParameterChanges(
        std::initializer_list<ParameterChangePoint> points) {
...
private:
    void allocateRingbuffers(size_t num_param_change_ring_buffer) {
        for (auto i = 0u; i < num_param_change_ring_buffer; ++i) {
            auto ringbuffer = std::make_shared<ParameterChangeRingbuffer>(
                kMaxParameterChanges);
    ...
    void insertNewRingBuffer(int param_index) {
        auto ringbuffer =
            std::make_shared<ParameterChangeRingbuffer>(kMaxParameterChanges);
