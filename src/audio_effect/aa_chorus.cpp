            ...
            for (int j = 0; j < num_voices - 1; ++j) {
                ...
                if (weight != 0.0f) {
                    ...
                    float delay_second =
                        min_delay +
                        sweep_width *
                            impl_->lfo_.lfo(fmodf(ph + phase_offset, 1.0f),
                                            LFO::WaveformType::kWaveformSine);
                    float delay_sample =
                        delay_second * static_cast<float>(getSampleRate());

                    // get interpolation value
                    float interpolation_val =
                        dline->getInterpolation(delay_sample);
