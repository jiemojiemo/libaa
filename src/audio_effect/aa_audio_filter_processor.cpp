...
public:
    ...
    int prepareToPlay(ProcessorPrepareConfig config){
        ...
        utilities_.updateCoeffes(
                ...
                param_.get(1).getPlainValue(),
                param_.get(2).getPlainValue(),
                param_.get(3).getPlainValue(),
                sample_rate_);
    ...
    int processBlock(AudioBlock *in_block, AudioBlock *output_block){
        ...
        for(auto c = 0u; c < output_block->audio_buffer.getNumberChannels(); ++c)
        {
            float* channel = output_block->audio_buffer.getWriterPointer(c);
            Biquad& b = utilities_.biquad_array[c];
            for(auto i = 0u; i < output_block->audio_buffer.getNumberFrames(); ++i)
            {
...
int AudioFilterProcessor::processBlock(AudioBlock *in_block, AudioBlock *output_block)  {
