...
Eigen::ArrayXXcf STFT::stft(const float *data, size_t data_len,
                            const Options &opts) {
    ...
    for (; idx < data_len - opts.win_size;) {
        std::copy(data + idx, data + idx + opts.win_size, cur_frame.data());
...
Eigen::ArrayXf STFT::istft(const Eigen::MatrixXcf &S, const Options &opts) {
    ...
    auto w_sum = Window::windowSum(opts.win_type, n_frames, opts.win_size,
                                   opts.hop_size);
