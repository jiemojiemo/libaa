...
float LevelDetector::process(float xn) {
    ...
    if (param_.mode == LevelDetectorMode::kLevelDetectorMode_MS ||
        ...
        input *= input;
        current_level =
            kMSAlpha * (last_level_ * last_level_) + (1 - kMSAlpha) * (input);
    } else // peak mode
    {
        // smooth, decoupled peak detector should be used(from: Digital Dynamic
        // Range Compressor Design— A Tutorial and Analysis)
