...
public:
    Impl(MP3FormatWriter *parent, int rate, int channels, int bits)
        : parent_(parent) {
    ...
    lame_global_flags *initLAME(int rate, int channels) {
    ...
    bool writePlanar(const float **samples, int num_samples) {
        ...
        if (parent_->num_channels == 1) {
            num_bytes_output = lame_encode_buffer_ieee_float(
                gfp_, samples[0], NULL, num_samples, mp3_buffer_.data(),
                mp3_buffer_.size());
        } else if (parent_->num_channels == 2) {
            num_bytes_output = lame_encode_buffer_ieee_float(
                gfp_, samples[0], samples[1], num_samples, mp3_buffer_.data(),
                mp3_buffer_.size());
        } else {
            return false;
        }

        return parent_->out_stream_->write(mp3_buffer_.data(),
                                           num_bytes_output);
    }
    ...
    void flush() {
        if (isOpen()) {
            int num_bytes_output =
                lame_encode_flush(gfp_, mp3_buffer_.data(), mp3_buffer_.size());
