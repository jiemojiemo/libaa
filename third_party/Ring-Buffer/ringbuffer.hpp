...
namespace jnk0le
{
	 ...
	 */
	template<typename T, bool fake_tso = false, size_t cacheline_size = 0, typename index_t = size_t>
		class Ringbuffer
		{
		public:
			/*!
			 ...
			 */
			Ringbuffer(size_t size) : buffer_size(size), buffer_mask(buffer_size-1), head(0), tail(0)
			{
			    assert(size > 0);
			    assert((buffer_size & buffer_mask) == 0); // power of 2
			    data_buff = new T[buffer_size];
            }
            ~Ringbuffer(){
			    delete data_buff;
			}
...
//				"buffer size is too large for a given indexing type (maximum size for n-bit type is 2^(n-1))");
	...
	template<typename T, bool fake_tso, size_t cacheline_size, typename index_t>
		size_t Ringbuffer<T, fake_tso, cacheline_size, index_t>::writeBuff(const T* buff, size_t count)
		{
			index_t available = 0;
			index_t tmp_head = head.load(std::memory_order_relaxed);
	...
	template<typename T, bool fake_tso, size_t cacheline_size, typename index_t>
		size_t Ringbuffer<T, fake_tso, cacheline_size, index_t>::readBuff(T* buff, size_t count)
		{
			index_t available = 0;
			index_t tmp_tail = tail.load(std::memory_order_relaxed);
